...
from src.models.DML_PCL.nn.utils import device,Averager
from src.models.DML_PCL.nn.densities import mixture_of_gaussian_loss
from src.models.DML_PCL.nn.architectures import FeedForward,MixtureGaussian,ConvNet
from src.models.DML_PCL.samplers import random_gmm
...
def train_Ymodel(Ymodel,train_data,valid_data,batch_size,epochs,lr,w_decay,verbose=1):
    ...
    valid_z=torch.tensor(valid_data[0]).to(device)
    valid_x=torch.tensor(valid_data[1]).to(device)
    ...
    for ep in range(epochs):
        ...
        if (ep + 1) % 10 == 0 or ep==0:  # To see training progress, we also conduct validation when 'iteration == 0'
...
class Ymodel(nn.Module):
    ...
    def __init__(self, input_channel, hiddens,dropout,use_image=False,image_shape=None):
        ...
        if use_image:
            self.net=ConvNet(hiddens,dropout=dropout,out_dim_add=3)
        else:
    ...
    def forward(self,inst,feat):
        if self.use_image:
            image=feat.reshape(self.image_shape)
            fnn = self.net(image,[inst])
        else:
...
def train_treatment(treatment_model,train_data,valid_data,batch_size,loss,epochs,lr,w_decay,verbose=1):
...
class Treatment(nn.Module):
    ...
    def __init__(self, input_channel, hiddens,dropout,n_components=10,use_image=False,image_shape=None):
    ...
    def forward(self,inst,feat):
        if self.use_image:
            image=feat.float().reshape(self.image_shape)
            fnn = self.net(image,[inst])
        else:
            inp=torch.cat((inst,feat),-1).float()
            fnn=self.fnn(inp)
...
def train_response_kfold(response_model,treatments,y_models,train_datasets,valid_data,
                         dml,batch_size,epochs,lr,w_decay,n_samples=1,verbose=1):
    ...
    for ep in range(epochs):
            ...
            for i, (inst, feat, treat,resp) in enumerate(train_loader):
                ...
                for s in range(n_samples):
                    with torch.no_grad():
                        ...
                        # pred_res = y_models[fold].sample(inst, feat, n_samples=1)
        ...
        if (ep + 1) % 10 == 0 or ep==0:  # To see training progress, we also conduct validation when 'iteration == 0'
            elapsed_time = time.time() - start_time
            # training loss and validation loss
            loss_log = f'[{ep + 1}/{epochs}] Train: {loss_avg.val():0.3f}, Custom:{custom_avg.val():0.3f},Reg:{reg_avg.val():0.3f}'
            loss_avg.reset()
            custom_avg.reset()
            reg_avg.reset()
            if verbose==1:
                print('k-fold response: ', loss_log)
    return response_model

def train_response(response_model,treatment_model,Y_model,train_data,valid_data,
                   dml,batch_size,epochs,lr,w_decay,n_samples=1,verbose=1):
        ...
        for i, (inst, feat, treat,resp) in enumerate(train_loader):
            ...
            for s in range(n_samples):
                with torch.no_grad():
                    ...
                    # pred_res=Y_model.sample(inst,feat,n_samples=1)
...
class Response(nn.Module):
    ...
    def __init__(self, input_channel, hiddens, dropout,use_image=False,image_shape=None):
        ...
        self.use_image=use_image
        if use_image:
            self.net=ConvNet(hiddens,dropout=dropout,out_dim_add=1)
        else:
            self.fnn = FeedForward(input_channel, hiddens, dropout)
