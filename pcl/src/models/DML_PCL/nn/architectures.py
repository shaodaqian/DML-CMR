...
class ConvNet(nn.Module):
    ...
    def __init__(self, hiddens=[128,64,32],
            pool_size=(2, 2), kernel_size=(5, 5),dropout=0.1,out_dim_add=1):
        super(ConvNet, self).__init__()
        # self.hiddens = hiddens

        self.Conv = nn.Sequential( #1x64x64
            ...
            nn.Flatten(1,-1),
            # nn.Dropout(p=dropout),
            nn.Linear(hiddens[1]*16, hiddens[1]),
            nn.ReLU(True),
            nn.Dropout(p=dropout),
        ...
        self.FeedForward = nn.Sequential( #1x20x100
            nn.Linear(hiddens[1]+out_dim_add, hiddens[2]),
            nn.ReLU(True),
            nn.Dropout(p=dropout),
        )
