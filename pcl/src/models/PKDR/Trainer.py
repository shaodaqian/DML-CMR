import yaml
import numpy as np
from src.models.PKDR.GPS import GPS
from src.models.PKDR.kernel import epanechnikov_kernel
from src.models.PKDR.rkhs.rkhs_approx import ApproxRKHSIVCV


class RKHS_Trainer():
    def __init__(self, dataset, gamma_gm, n_gamma_hqs, n_alphas, alpha_scales, cv, n_components,**kwargs):
        """
    ...
    def fit_q_cv(self, type='kde', hyperparams=None):
        """
    ...
    def _prepare_input(self, point, W, X=None):
        ...
        """
        point_full = np.repeat(np.expand_dims(point,0), len(W), axis=0)
        ...
        for a in pointA:
            if method == 'h':
                ...
                ATE = np.mean((Y - self.h(inp_h)) * q_azx *
                              epanechnikov_kernel((A - a), bandwidth).mean(-1) + self.h(inp_h))
