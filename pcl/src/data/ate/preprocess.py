...
class AbstractPreprocessor:
    ...
    def preprocess_for_val_input(self, val_data: PVTrainDataSet, **kwarg) -> PVTrainDataSet:
        raise NotImplementedError
...
class ScaleAllPreprocessor(AbstractPreprocessor):
    ...
    def preprocess_for_train(self, train_data: PVTrainDataSet, **kwarg) -> PVTrainDataSet:
        self.treatment_proxy_scaler = StandardScaler()
        treatment_proxy_s = self.treatment_proxy_scaler.fit_transform(train_data.treatment_proxy)
        ...
        self.outcome_proxy_scaler = StandardScaler()
        outcome_proxy_s = self.outcome_proxy_scaler.fit_transform(train_data.outcome_proxy)

        backdoor_s = None
        if train_data.backdoor is not None:
            self.backdoor_scaler = StandardScaler()
            backdoor_s = self.backdoor_scaler.fit_transform(train_data.backdoor)
    ...
    def preprocess_for_val_input(self, val_data: PVTrainDataSet, **kwarg) -> PVTrainDataSet:
        treatment_proxy_s = self.treatment_proxy_scaler.transform(val_data.treatment_proxy)
        treatment_s = self.treatment_scaler.transform(val_data.treatment)
        outcome_s = self.outcome_scaler.transform(val_data.outcome)
        outcome_proxy_s = self.outcome_proxy_scaler.transform(val_data.outcome_proxy)

        backdoor_s = None
        if val_data.backdoor is not None:
            backdoor_s = self.backdoor_scaler.transform(val_data.backdoor)
